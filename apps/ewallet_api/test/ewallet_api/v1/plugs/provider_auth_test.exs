defmodule EWalletAPI.V1.Plug.ProviderAuthTest do
  ...
  describe "V1.Plugs.ProviderAuth with PLAYServer auth type" do
  ...
  describe "V1.Plugs.ProviderAuth with invalid auth scheme" do
    ...
    test "halts with :invalid_auth_scheme if credentials format is invalid" do
      conn =
        build_conn()
        |> put_auth_header("PLAYServer", "not_colon_separated_base64")
        |> ProviderAuth.call([])
  ...
  defp invoke_conn(api_key, auth_token), do: invoke_conn("PLAYServer", api_key, auth_token)
