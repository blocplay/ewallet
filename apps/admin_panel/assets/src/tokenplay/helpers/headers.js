...
function authenticationHeader(authenticated) {
  ...
  if (authenticated && authenticationToken) {
    return {
      Authorization: `PLAYAdmin ${btoa(`${ADMIN_API_KEY_ID}:${ADMIN_API_KEY}:${authenticationToken}`)}`,
    };
  }
  return { Authorization: `PLAYAdmin ${btoa(`${ADMIN_API_KEY_ID}:${ADMIN_API_KEY}`)}` };
}

function accountHeader() {
  const currentAccountId = Cookies.get(sessionConstants.ACCOUNT_COOKIE);
  return currentAccountId ? { 'PLAYAdmin-Account-ID': currentAccountId } : {};
}
