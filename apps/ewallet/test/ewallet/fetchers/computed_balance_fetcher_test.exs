defmodule EWallet.ComputedBalanceFetcherTest do
  ...
  describe "all/1" do
    test "retrieve all balances from a provider_user_id" do
      ...
      {:ok, play}     = :minted_token |> params_for(symbol: "PLAY") |> MintedToken.insert()
      ...
      assert address.balances == [
        ...
        %{minted_token: play, amount: 12_000 * play.subunit_to_unit},
  ...
  describe "get/2" do
    test "retrieve the specific balance from a minted_token and an address" do
      ...
      {status, address} = ComputedBalanceFetcher.get(play.friendly_id, user_balance.address)
      assert status == :ok
