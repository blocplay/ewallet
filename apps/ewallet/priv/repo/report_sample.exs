...
CLI.print("""
  ...
  ```
  PLAYServer #{Base.encode64(ewallet_key.access_key <> ":" <> ewallet_key.secret_key)}
  ```
  ...
  ```
  PLAYClient #{Base.encode64(ewallet_api_key.key <> ":" <> ewallet_auth_token)}
  ```
  ...
  ```
  PLAYAdmin #{Base.encode64(admin_api_key.id <> ":" <> admin_api_key.key)}
  ```
  ...
  ```
  PLAYAdmin #{Base.encode64(admin_api_key.id <> ":" <> admin_api_key.key <> ":" <> admin_user.id
    <> ":" <> admin_auth_token)}
  ```
