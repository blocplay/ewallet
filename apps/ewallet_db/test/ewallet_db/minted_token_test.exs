defmodule EWalletDB.MintedTokenTest do
  ...
  describe "insert/1" do
    ...
    test "generates a friendly_id" do
      {:ok, minted_token} =
        :minted_token |> params_for(friendly_id: nil, symbol: "PLAY") |> MintedToken.insert

      assert "PLAY:" <> uuid = minted_token.friendly_id
    ...
    test "does not regenerate a friendly_id if already there" do
      {:ok, minted_token} =
        :minted_token |> params_for(friendly_id: "PLAY:123") |> MintedToken.insert

      assert minted_token.friendly_id == "PLAY:123"
    end
  end
